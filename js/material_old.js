...
function initExpansionPanels() {
	...
	for (var i = 0; i < panels.length; i++) {
		...
		primary[0].textContent = primary[0].dataset.closedText;
		secondary[0].textContent = secondary[0].dataset.closedText;
		...
		extend.addEventListener("mousedown",
			function() {
				var el = $(this).closest("div.expansion-panel");
		        el[0].classList.toggle("open");

                var primary = el.find("div.summary.primary");
                var secondary = el.find("div.summary.secondary");

                primary[0].textContent = primary[0].dataset.openText;
                secondary[0].textContent = secondary[0].dataset.openText;
			}
		);
